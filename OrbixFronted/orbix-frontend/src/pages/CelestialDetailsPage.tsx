...
function CelestialDetailsPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const mainEndpoint = isSunRoute ? '/info/sun' : `/info/planet/${id}`
            const specializedEndpoint = isSunRoute
                ? '/info/jpl/specialized/sun'
                : `/info/jpl/specialized/planet/${bodyName}`
            const mediaEndpoint = `/info/nasa-media/${bodyName}`
            ...
            try {
                ...
                console.group(`[DETAIL PAGE] ${bodyName}`)
                console.log(`Iniciando extracción de datos endpoint principal: ${mainEndpoint}`)
                ...
                if (!cancelled) {
                    ...
                    console.log(`Extracción exitosa endpoint principal: ${mainEndpoint}`)
                    console.log('Respuesta endpoint principal =>', primaryResponse)
                }

                console.log(`Finalizó extracción endpoint principal: ${mainEndpoint}`)

                console.log(`Iniciando extracción de datos endpoint especializado: ${specializedEndpoint}`)
                const specializedPromise = isSunRoute
                    ? getSunSpecializedInfo()
                    : getPlanetSpecializedInfoByName(bodyName)

                console.log(`Iniciando extracción de datos endpoint media: ${mediaEndpoint}`)
                const mediaPromise = getPlanetMediaByName(bodyName)

                const [specializedResult, mediaResult] = await Promise.allSettled([
                    specializedPromise,
                    mediaPromise,
                ])

                if (cancelled) {
                    console.warn(`[DETAIL PAGE] Solicitudes canceladas para ${bodyName}`)
                    console.groupEnd()
                    return
                }

                if (specializedResult.status === 'fulfilled') {
                    setSpecializedData(specializedResult.value)
                    console.log(`Extracción exitosa endpoint especializado: ${specializedEndpoint}`)
                    console.log('Respuesta endpoint especializado =>', specializedResult.value)
                } else {
                    console.error(`Error en extracción endpoint especializado: ${specializedEndpoint}`)
                    console.error(specializedResult.reason)
                }

                console.log(`Finalizó extracción endpoint especializado: ${specializedEndpoint}`)

                if (mediaResult.status === 'fulfilled') {
                    setMediaData(mediaResult.value)
                    console.log(`Extracción exitosa endpoint media: ${mediaEndpoint}`)
                    console.log('Respuesta endpoint media =>', mediaResult.value)
                } else {
                    console.error(`Error en extracción endpoint media: ${mediaEndpoint}`)
                    console.error(mediaResult.reason)
                }

                console.log(`Finalizó extracción endpoint media: ${mediaEndpoint}`)
                console.groupEnd()
            } catch (err) {
                if (!cancelled) {
                    setError('Unable to load celestial details.')
                    setData(null)
                    console.error(`Error general cargando detalle de ${bodyName}:`, err)
                    console.groupEnd()
                }
            } finally {
                if (!cancelled) {
                        ...
                        <div className="mt-5 rounded-[20px] border border-white/8 bg-white/5 p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <p className="text-sm text-slate-200">
                                    {loading
                                        ? 'Loading main backend response...'
                                        : extraLoading
                                            ? 'Main data loaded, extracting specialized datasets...'
                                            : error
                                                ? error
                                                : 'All backend requests completed'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="mt-4 rounded-[20px] border border-white/8 bg-white/5 p-4">
                            ...
                            <p className="mt-3 text-sm leading-7 text-slate-300">
                                {loading
                                    ...
                                    : extraLoading
                                        ? 'The detail view is open and now requesting the specialized JPL and NASA media datasets in the background.'
                                        : error
